...
func MakeHandler(s Service) http.Handler {
	...
	mux.Handle("/sales/register", m.Handler(makeRegisterSaleEndpoint(s)))
	mux.Handle("/list/sales", m.Handler(makeSaleConfirmList(s)))
	mux.Handle("/get/sales", m.Handler(makeGetSalesPersonEndpoint(s)))
	mux.Handle("/get/sale/byuser", m.Handler(makeGetSaleByUser(s)))
	mux.Handle("/get/sale/commision", m.Handler(makeGetSaleByUserV2Endpoint(s)))
	mux.Handle("/confirm/sales", m.Handler(makeConfrimSaleEndpoint(s)))
	mux.Handle("/update/sales", m.Handler(makeUpdateSaleEndpoint(s)))
	mux.Handle("/update/sales/byadmin", m.Handler(makeUpdateSaleByAdmin(s)))
	mux.Handle("/remove/sales", m.Handler(DeleteSalesEnpdoint(s)))
	mux.Handle("/get/sales/team", m.Handler(MakeEnpointGetSalesTeam(s)))
	mux.Handle("/genarte/commision", m.Handler(makeGenarateCommision(s)))
	mux.Handle("/list/commision", m.Handler(makeListCommision(s)))
	mux.Handle("/get/commision/docno", m.Handler(makeGetCommision(s)))

	mux.Handle("/list/commision/all", m.Handler(MakeEndpointListCommision(s)))
	mux.Handle("/update/approve/commision", m.Handler(MakeEndpointApprove(s)))
	mux.Handle("/get/commision/admin/docno", m.Handler(MakeEnpointGetCommsionDocNoBackEnd(s)))

	mux.Handle("/cancel/commision/admin/docno", m.Handler(MakeEnpointGetCommsionDocNoBackEnd(s)))
	
	mux.Handle("/cancel/commision/user/docno", m.Handler(MakeEnpointGetCommsionDocNoBackEnd(s)))
